...
This repository is a [Copier](https://copier.readthedocs.io/) template. Generate a new project with:

```bash
uvx copier copy gh:danielgatis/go-reasonable-api my-new-api
```

or, if you already have Copier installed:

```bash
copier copy gh:danielgatis/go-reasonable-api my-new-api
```

Copier will ask for a handful of values (`project_name`, `module_path`, `brand_name`, etc.) with sensible defaults, then it rewrites Go imports to your module path and runs `go mod tidy`. To pull future template updates into the generated project, run `copier update` from inside it.
...
After generating a project with Copier, inside the new directory:
...
Task type names follow the **`resource:action`** convention, all lowercase:

| Task | Type string |
|------|-------------|
| Email sending | `email:send` |
| Periodic maintenance cleanup | `maintenance:cleanup` |

Pick names like `user:archive`, `report:generate`, `webhook:deliver`. Keep the resource on the left and the verb on the right — handler registration, logs and Asynq's dashboards all surface this string, so consistency pays off.
...
```bash
make build
./bin/<project_slug> api      # Start API server
./bin/<project_slug> worker   # Start background worker
```
